...
module RR
  ...
  module ConnectionExtenders
    ...
    def self.db_connect_without_cache(config)
      ...
      if RUBY_PLATFORM =~ /java/ and config[:driver]=='com.ibm.db2.jcc.DB2Driver'
        connection.extend RR::ConnectionExtenders::DB2Extender
        replication_module = ReplicationExtenders.extenders[:db2]
        connection.raw_connection.connection.autoCommit=false
        
        if config[:schema]
          puts "Setting schema to #{config[:schema]}"
          connection.execute("set schema #{config[:schema]}")
          options = RR::Configuration.instance.options
        
          connection.execute("create variable #{config[:schema]}.#{options[:rep_prefix]}_app_var varchar(100) default ''") rescue nil
          connection.commit_db_transaction
          connection.execute("set #{config[:schema]}.#{options[:rep_prefix]}_app_var=application_id()")          
        end           
      else
