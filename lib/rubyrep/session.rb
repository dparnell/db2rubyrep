  ...
  class Session
    ...
    def database_unreachable?(database)
      ...
      Thread.new do
        begin
          if db=send(database)
            a = db.connection.adapter_name rescue nil
            if a=='db2' or (a=='JDBC' and db.connection.raw_connection.connection.getMetaData.getDatabaseProductName[0..2]=='DB2')
              check_sql = 'select 1+1 as x from sysibm.sysdummy1'
            else
              check_sql = 'select 1+1 as x'
            end
          
            if send(database).select_one(check_sql)['x'].to_i == 2
