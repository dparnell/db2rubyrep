  ...
  class ReplicationInitializer
    ...
    def create_event_log
      silence_ddl_notices(:left) do
        ...
        session.left.connection.create_table "#{options[:rep_prefix]}_logged_events" do |t|
    ...
    def create_change_log(database)
      silence_ddl_notices(database) do
        ...
        connection.connection.create_table table_name do |t|
          t.column :change_table, :string
          t.column :change_key, :string
    ...
    def ensure_activity_markers
      ...
      [:left, :right].each do |database|
        ...
        unless connection.tables.include? table_name
          silence_ddl_notices(database) do
            connection.connection.create_table(table_name.to_sym, :id => false) do |t|
    ...
    def restore_unconfigured_tables(configured_table_pairs = session.configured_table_pairs)
      [:left, :right].each do |database|
        configured_tables = configured_table_pairs.map {|table_pair| table_pair[database].downcase}
        ...
        unconfigured_tables.each do |table|
          if trigger_exists?(database, table)
            puts "Removing triggers on #{database.inspect} #{table}"
    ...
    def prepare_replication
      ...
      # TODO: add code to allow the table verification to be turned off as it only needs to be run once
...
#        puts "checking: #{table_pair.inspect}"
        table_options = options(table_pair[:left])
